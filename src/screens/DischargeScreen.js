...
// Shown after a doctor saves a SOAP note.
// Doctor can: schedule a follow-up OR discharge immediately.
        ...
        <View key={ri} style={cal.row}>
          {Array.from({ length: 7 }).map((_, di) => {
            ...
            return (
              <TouchableOpacity
                key={di}
                style={[cal.cell, sel && cal.selectedCell, past && cal.pastCell, !day && { opacity: 0 }]}
                onPress={() => handleDay(day)}
                disabled={!day || past}
              >
                <Text style={[cal.cellText, sel && cal.selectedCellText, past && cal.pastCellText]}>
                  {day || ''}
                </Text>
              </TouchableOpacity>
            );
...
export default function DischargeScreen({
  ...
  const [mode, setMode] = useState(null); // null | 'appointment' | 'discharging'
    ...
    try {
      ...
      Alert.alert(
        ...
        [{
          ...
          onPress: () => onDischarge?.({ scheduled: true }),
        }],
      );
    } catch (e) {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>
          What would you like to do with {patient?.firstName} {patient?.lastName}?
        </Text>
      </View>
        ...
        <View style={styles.optionRow}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              Alert.alert(
                'Discharge Patient',
                `Discharge ${patient?.firstName} without scheduling a follow-up?`,
                [
                  { text: 'Cancel', style: 'cancel' },
                  { text: 'Discharge', onPress: handleDischargeNow },
                ],
              );
            }}
            activeOpacity={0.85}
          >
            ...
            <Text style={styles.optionSubtitle}>
              Discharge without a follow-up appointment
            </Text>
          </TouchableOpacity>
        </View>
      )}
          ...
          {!hasEmail && (
            <View style={styles.noEmailWarning}>
              ...
              <Text style={styles.noEmailWarningText}>
                Patient has no email on file — confirmation email cannot be sent.
                {' '}Update their profile to add one.
              </Text>
            </View>
          )}
          ...
          <View style={styles.modalSheet} onStartShouldSetResponder={() => true}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={[styles.timeChipText, selectedTime === item && styles.timeChipTextActive]}>
                    {item}
                  </Text>
                </TouchableOpacity>
              )}
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setReasonPickerVisible(false)}>
          <View style={styles.modalSheet} onStartShouldSetResponder={() => true}>
            ...
            {APPOINTMENT_REASONS.map(r => (
              ...
              >
                <Text style={[styles.reasonOptionText, selectedReason === r && styles.reasonOptionTextActive]}>
                  {r}
                </Text>
...
const styles = StyleSheet.create({
  ...
  optionRow: { gap: 14 },
  optionCard: {
    borderRadius: 16, padding: 24, borderWidth: 1.5,
  ...
  optionTitle: { fontSize: 18, fontWeight: '800', color: '#0f766e', marginBottom: 6 },
  ...
  modalSheet: {
    ...
    padding: 20, maxHeight: '70%', paddingBottom: Platform.OS === 'ios' ? 40 : 24,
  },
