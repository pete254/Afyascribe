// App.js - Fixed safe area + drill-down navigation
...
// ── Bottom Tab Bar ─────────────────────────────────────────────────────────────
function BottomTabBar({ activeTab, user, goToHome, goToMyQueue, goToQueue, goToSavedNotes, onPressNewNote }) {
  const insets = useSafeAreaInsets();
  const bottomPad = Math.max(insets.bottom, 8);

  return (
    <View style={[styles.tabBar, { paddingBottom: bottomPad }]}>
      <TouchableOpacity style={[styles.tab, activeTab === 'home' && styles.activeTab]} onPress={goToHome}>
        <Ionicons name={activeTab === 'home' ? 'home' : 'home-outline'} size={22} color={activeTab === 'home' ? '#0f766e' : '#94a3b8'} />
        <Text style={[styles.tabText, activeTab === 'home' && styles.activeTabText]}>Home</Text>
      </TouchableOpacity>

      {user?.role === 'doctor' ? (
        <TouchableOpacity style={[styles.tab, activeTab === 'my-queue' && styles.activeTab]} onPress={goToMyQueue}>
          <MaterialCommunityIcons name="account-clock-outline" size={22} color={activeTab === 'my-queue' ? '#0f766e' : '#94a3b8'} />
          <Text style={[styles.tabText, activeTab === 'my-queue' && styles.activeTabText]}>My Queue</Text>
        </TouchableOpacity>
      ) : (
        <TouchableOpacity style={[styles.tab, activeTab === 'queue' && styles.activeTab]} onPress={goToQueue}>
          <MaterialCommunityIcons name="clipboard-list-outline" size={22} color={activeTab === 'queue' ? '#0f766e' : '#94a3b8'} />
          <Text style={[styles.tabText, activeTab === 'queue' && styles.activeTabText]}>Queue</Text>
        </TouchableOpacity>
      )}

      <TouchableOpacity style={[styles.tab, activeTab === 'transcription' && styles.activeTab]} onPress={onPressNewNote}>
        <MaterialCommunityIcons name="microphone" size={22} color={activeTab === 'transcription' ? '#0f766e' : '#94a3b8'} />
        <Text style={[styles.tabText, activeTab === 'transcription' && styles.activeTabText]}>New Note</Text>
      </TouchableOpacity>

      <TouchableOpacity style={[styles.tab, activeTab === 'saved' && styles.activeTab]} onPress={goToSavedNotes}>
        <MaterialCommunityIcons name={activeTab === 'saved' ? 'note-text' : 'note-text-outline'} size={22} color={activeTab === 'saved' ? '#0f766e' : '#94a3b8'} />
...
function MainApp() {
  ...
  // ── Nav helpers ──────────────────────────────────────────────────────────────
  const goToHome = () => {
    setActiveScreen('home'); setActiveTab('home');
    setNoteToEdit(null); setSelectedPatient(null);
    setTriageVisit(null); setSoapVisit(null);
  };

  const goBack = () => {
    const fullScreens = ['my-queue', 'queue'];
    if (fullScreens.includes(activeTab) || fullScreens.includes(activeScreen)) { goToHome(); return; }
    const returnTo = activeTab === 'home' ? 'home' : activeTab;
    setActiveScreen(returnTo);
    setSelectedPatient(null); setNoteToEdit(null);
    setTriageVisit(null); setSoapVisit(null);
  };

  const goToPatientHistory = (patient) => { setSelectedPatient(patient); setActiveScreen('patient-history'); };
  const goToTranscription = (patient = null) => {
    if (patient) setSelectedPatient(patient);
    setActiveScreen('transcription'); setActiveTab('transcription');
  };
  const editNoteWithVoice = (note) => {
    setNoteToEdit(note); setSelectedPatient(note.patient);
    setActiveScreen('transcription'); setActiveTab('transcription');
  };
  const goToSavedNotes = () => { setActiveScreen('saved'); setActiveTab('saved'); };
  ...
  const goToQueue = () => { setActiveScreen('queue'); setActiveTab('queue'); };
  const goToMyQueue = () => { setActiveScreen('my-queue'); setActiveTab('my-queue'); };
  const goToReports = () => setActiveScreen('reports');
  const goToServiceCatalog = () => setActiveScreen('service-catalog');
  const goToTriage = (visit = null) => { setTriageVisit(visit); setViewTriageOnly(false); setActiveScreen('triage'); };
  const goToViewTriage = (visit) => { setTriageVisit(visit); setViewTriageOnly(true); setActiveScreen('triage'); };
  const openSoapFromQueue = (visit) => {
    setSoapVisit(visit); setSelectedPatient(visit.patient);
    setActiveScreen('transcription'); setActiveTab('transcription');
  };
  ...
  const hideTabBar = [
    ...
    'service-catalog', 'discharge', 'prescription', 'owner-card',
  ...
  const renderScreen = () => {
    switch (activeScreen) {
      ...
      case 'queue-patient':
        return <QueuePatientScreen onBack={goBack} onSuccess={goToHome} />;
      case 'queue':
        return <QueueScreen onBack={goBack} onTriagePatient={(visit) => goToTriage(visit)} />;
      case 'my-queue':
        return (
          <MyQueueScreen
            onBack={goBack}
      ...
      case 'onboard-patient':
        return <OnboardPatientScreen onBack={goBack} onSuccess={goToHome} />;
      case 'service-catalog':
        return <ServiceCatalogScreen onBack={goBack} />;
      case 'patient-directory':
        return <PatientDirectoryScreen onBack={goBack} onViewPatientHistory={goToPatientHistory} />;
      ...
      case 'discharge':
        return (
          <DischargeScreen
            ...
            onWritePrescription={() => { setPrescriptionContext(dischargeContext); setActiveScreen('prescription'); }}
            onDischarge={() => { setDischargeContext(null); setPrescriptionContext(null); goToHome(); }}
      ...
      case 'prescription':
        return (
          <PrescriptionScreen
            ...
            onDone={() => { setPrescriptionContext(null); setDischargeContext(null); goToHome(); }}
          />
        );
      case 'transcription':
      default:
        return (
          <TranscriptionScreen
            ...
            onSoapNoteSaved={(ctx) => { setDischargeContext(ctx); setActiveScreen('discharge'); }}
          />
        );
    }
  };
...
const styles = StyleSheet.create({
  ...
  // Bottom tab bar — paddingBottom is dynamic from insets
  tabBar: {
    flexDirection: 'row',
  ...
  tab: {
    ...
    paddingVertical: 4, borderRadius: 12, marginHorizontal: 2,
  },
